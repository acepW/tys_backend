...
class ContractService extends DualDatabaseService {
  ...
  // 🔥 Ambil seluruh riwayat versi (root + semua turunan) untuk sekumpulan contract sekaligus
  async _attachVersionHistory(rows, dbModels) {
    if (!rows || rows.length === 0) return rows;

    const plainRows = rows.map((r) => (r.toJSON ? r.toJSON() : r));

    // Resolve rootId tiap row: kalau id_root_contract null berarti dia sendiri root
    const rootIds = [
      ...new Set(plainRows.map((r) => r.id_root_contract || r.id)),
    ];

    // Ambil semua row (root + turunannya) dalam satu query
    const allChainVersions = await dbModels.Contract.findAll({
      where: {
        [Op.or]: [{ id: rootIds }, { id_root_contract: rootIds }],
      },
      attributes: [
        "id",
        "contract_no",
    ...
    for (const v of allChainVersions) {
      const key = v.id_root_contract || v.id;
      if (!historyMap[key]) historyMap[key] = [];
      historyMap[key].push(v);
    }

    // Tempel ke tiap row, exclude dirinya sendiri
    return plainRows.map((row) => {
      const rootId = row.id_root_contract || row.id;
      const history = historyMap[rootId] || [];
      row.derived_versions = history.filter((v) => v.id !== row.id);
      return row;
    });
  }
  ...
  async getAllWithRelations(
    ...
    const queryOptions = {
      ...
      include: [
        ...
        // ❌ derived_versions dihapus dari sini, akan diisi manual setelah query
        {
        ...
        { model: dbModels.PreOrder, as: "pre_orders" },
        {
          ...
          include: [
            { model: dbModels.QuotationService, as: "quotation_service" },
            {
              ...
              include: [
                ...
                { model: dbModels.PaymentRequest, as: "payment_requests" },
                {
                  model: dbModels.User,
                  as: "user_started",
                  attributes: ["id", "name", "email"],
    ...
    const rows = page && limit ? result.rows : result;
    const rowsWithHistory = await this._attachVersionHistory(rows, dbModels);

    if (!page || !limit) {
      return rowsWithHistory;
    }
  ...
  async getById(id, options = {}, isDoubleDatabase = true) {
    ...
    const queryOptions = {
      ...
      include: [
        ...
        // 🔥 Info ringkas semua versi sebelumnya
        {
          model: dbModels.Contract,
          as: "derived_versions",
          attributes: ["id", "contract_no", "version", "is_active"],
  ...
  async createWithRelations(
    ...
    try {
      if (isDoubleDatabase) {
        ...
        // 5. Process Contract Clause header
        const clauseHeaderResult = [];
        for (const clauseHeaderData of clause_header) {
          const { ...clauseHeader } = clauseHeaderData;

          const clauseHeaderDataToCreate = {
            ...clauseHeader,
            id_contract: contract1.id,
          };
          const clauseHeader1 = await models.db1.ContractClauseHeader.create(
  ...
  async updateWithRelations(
    ...
    try {
      if (isDoubleDatabase) {
        ...
        // 3. Sync Contract Clause Headers (flat, no children)
        const clauseHeaderData = clause_header.map((header) => {
          const { id: _hid, ...cleanHeader } = header;
          return { ...header, id_contract: id };
        });

        const clauseHeaderResult = await syncChildRecords({
          Model1: models.db1.ContractClauseHeader,
          Model2: models.db2.ContractClauseHeader,
          foreignKey: "id_contract",
          parentId: id,
        ...
        for (const clauseData of clausesToUpdate) {
          ...
          for (const pointId of pointIdsToDelete) {
            ...
            // Delete clause logs for this point
            await models.db1.ContractClauseLog.destroy({
              where: { id_contract_clause_point: pointId },
              transaction: transaction1,
            });
        ...
        // Sync Contract Clause Headers (flat, no children)
        const clauseHeaderData = clause_header.map((header) => ({
          ...header,
          id_contract: id,
        }));
